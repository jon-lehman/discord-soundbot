...
class SearchCommand extends Command_1.default {
    ...
    run(message, params) {
        ...
        const results = SoundUtil_1.getSounds().filter(sound => sound.includes(tag));
        sounds.withTag(tag).forEach(sound => results.push(sound));
        if (!results.length) {
            message.author.send(localize_1.default.t('commands.search.notFound'));
            return;
        }
